...
def adicionar_vaga(entry_numero_vaga, entry_tipo_veiculo, var_disponibilidade):
...
def gerenciamento_vagas():
    ...
    tk.Label(window, text="Número da Vaga:").grid(row=0, column=0)
    entry_numero_vaga = tk.Entry(window)
    entry_numero_vaga.grid(row=0, column=1)

    tk.Label(window, text="Tipo de Veículo:").grid(row=1, column=0)
    entry_tipo_veiculo = tk.Entry(window)
    entry_tipo_veiculo.grid(row=1, column=1)

    tk.Label(window, text="Disponibilidade:").grid(row=2, column=0)
    var_disponibilidade = tk.IntVar(value=0)
    tk.Radiobutton(window, text="Livre", variable=var_disponibilidade, value=0).grid(row=2, column=1)
    tk.Radiobutton(window, text="Ocupada", variable=var_disponibilidade, value=1).grid(row=2, column=2)

    tk.Button(window, text="Adicionar Vaga", command=lambda: adicionar_vaga(entry_numero_vaga, entry_tipo_veiculo, var_disponibilidade)).grid(row=3, columnspan=3)
    ...
    tk.Button(window, text="Excluir Vaga", command=excluir_vaga).grid(row=5, column=0)
    tk.Button(window, text="Alterar Status", command=alterar_status_vaga).grid(row=5, column=1)
