...
def exibir_historico_pagamentos():
    try:
        ...
        if conn:
            ...
            query = """
                SELECT p.cod_pagamento, p.cod_ocupacao, p.cod_cliente, p.valor_pag, p.forma_pag, p.data_pag, o.cod_vaga
                FROM pagamentos p
                JOIN ocupacao o ON p.cod_ocupacao = o.cod_ocup
            ...
            headers = ["Código Pagamento", "Código Ocupação", "Código Cliente", "Valor", "Forma de Pagamento", "Data do Pagamento", "Código Vaga", "Excluir"]
